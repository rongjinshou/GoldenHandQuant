...
class TestOrder:
    ...
    def test_cancel_filled_order_should_raise_error(self):
        # Arrange
        order = Order(
            order_id="1",
            account_id="acc",
    ...
    def test_reject_created_order_should_raise_error(self):
        # Arrange
        order = Order(
            order_id="1",
            account_id="acc",
        ...
        # Act & Assert — CREATED 状态不可 reject，必须先 submit
        with pytest.raises(RuntimeError, match="Cannot reject order in status"):
            order.reject(reason="Not submitted")

    def test_fill_canceled_order_should_raise_error(self):
        # Arrange
        order = Order(
            order_id="1",
            account_id="acc",
    ...
    def test_cancel_rejected_order_should_raise_error(self):
        # Arrange
        order = Order(
            order_id="1",
            account_id="acc",
        ...
        order.submit()
        order.reject(reason="Rejected by exchange")
