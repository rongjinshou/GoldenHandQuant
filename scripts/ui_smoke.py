...
# 每页签: (tab key, 截图文件名, 就绪锚点列表[满足其一即可], 额外说明)
TABS = [
    ("overview", "01-overview", ["#overview-cards .card", "#overview-empty:not(.hidden)"]),
    ("verdicts", "02-verdicts", ["#ft-factors input[type=checkbox]"]),
    ("explorer", "03-explorer", ["#kline-chart canvas"]),
    ("backtests", "04-backtests", ["#bt-strategies input[type=checkbox]"]),
    ("live", "05-live", ["#live-ops-cards .card", "#live-empty:not(.hidden)"]),
    ("jobs", "06-jobs", ["#jobs-table"]),
]
...
def open_details(page) -> None:
    """展开当前激活页签里所有 <details> 表单卡, 让截图能看到表单全貌。"""
    page.evaluate(
...
def run_smoke(base: str, out_dir: Path, deep: bool) -> int:
    ...
    with sync_playwright() as pw:
        ...
        time.sleep(1.5)  # 首屏 Promise.all 数据加载
        ...
        for tab, fname, anchors in TABS:
            ...
            if tab == "explorer":
                ...
                page.fill("#symbol-input", "000021.SZ")
                page.click("#load-symbol")
...
def run_deep(page, out_dir: Path) -> dict:
    ...
    page.click('.tab[data-tab="backtests"]')
    open_details(page)
