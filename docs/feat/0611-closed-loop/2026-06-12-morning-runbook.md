...
> **2026-06-11 深夜更新**：①夜审完成 11 项修复（含一个会让守护循环明早静默全失败的
> 线程 bug），报告见 `2026-06-11-night-review.md`；②修复版守护进程已于 19:11 重新上线
> （dry-run，QMT 已连接），**晨间无需手动执行第 1 步，09:35 自动触发**，直接看驾驶舱实盘页即可；
> ③数据库已刷到 2026-06-11（含基本面+特征 v1 全量重算）；④夜间研究批次（P0 重判/P1 首测/
> F04 调仓敏感性）判决已入库——驾驶舱「因子判决」页查收；⑤回测新增 DuckDB 数据源
> （`backtest.yaml` 配 `data.history_fetcher: DuckDBHistoryDataFetcher`），QMT 不在线也能跑。
