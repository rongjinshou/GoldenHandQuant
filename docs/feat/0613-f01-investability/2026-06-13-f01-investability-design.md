...
| B7 | **回测重算技术指标且口径与研究库不一致（重要）**：回测走 `CrossSectionBuilder._compute_bar_metrics`（手写、120 根窗口重启 EMA）现算，而 `stock_features` 已由 `feature_engine`（向量化、`FEATURE_VERSION=1`）预计算并存库（factor-test 读它）。两者 (a) 重复计算（性能），(b) **口径不一致**——`feature_engine` 文档载明 v1 已修正手写版 `return_20d`(119/120 日)与 `macd`(窗口重启) 偏差，即回测用的是被承认有偏差的旧实现 ⇒ 回测技术指标 ≠ 研究库。本 Spec 已让 MicroValue 跳过（不用技术指标）；**根治（独立 Spec）**：回测改读 `stock_features`、废弃 `_compute_bar_metrics`、口径统一到 `feature_engine`，并验证 MultiFactor/ML 回测结果变化 |
| B8 | **因子检验/回测全程单线程（36 核只用 1 核，2026-06-14 用户点名）**：`prepare_snapshots` 逐日逐股 Python 建 `StockSnapshot`(~580 万对象, 峰值 ~12GB) + `LayerBacktester` 逐因子串行, 纯 Python 受 GIL 限单核; DuckDB 读可多核但非瓶颈。**修法**: ① `run_batch` 按因子 `ProcessPoolExecutor` 并行(因子间独立; 须避免重 pickle 大 snapshots dict→worker 各自从 `stock_features` 装载或用共享内存); ② prep 向量化(pandas, 仿 `feature_engine`)替代逐对象 Python(同时省内存); ③ 按日期块并行 prep。36 核潜在近线性提速, 是当前最大算力浪费 |
