...
- [ ] application 层**无模块级** infrastructure import(核心业务依赖经 TYPE_CHECKING / 归位 domain 彻底解耦;展示与边缘适配器 progress/plotter/config 用函数内延迟 import,运行时按需、模块级解耦)
...
深入实现后基于事实做了务实收敛——核心原则:**治理 over-engineering 不应制造新的 over-engineering**。

- **G4b(config_app)**:经核实 config_app 是 ConfigHotReloadService 的薄协调包装、二者紧耦合(测试即二者集成测试)、且**无生产调用方**。完整 DI 注入(Protocol + 注入 + 改 9 测试)对这种边缘薄包装是过度抽象。**改为** future annotations + 函数内延迟 import,消除模块级静态耦合,API 兼容、测试不改。完整注入待其真正接入生产 composition root 时再做。
- **G5(progress/plotter)**:backtest_app 的 BacktestProgress/BacktestPlotter 本就是**函数内延迟 import**(模块级已解耦)。它们是展示适配器,为展示关注点定义 Protocol + 注入收益边际。**保持延迟 import**,不做完整注入。
- **分级原则**:核心业务依赖(RiskSettings、build_cross_section)彻底解耦;展示与边缘适配器仅要求模块级解耦。这是基于"使用场景重要性 + 改造性价比"的工程判断。
