...
### 4.5 Phase 2：B7（回测复用列式源）— 范围在 Phase 1 后细化

调用点是 `BacktestStrategyRunner.evaluate`（`strategy_runner.py:188`），它**仅在 `strategy.uses_bar_history`
为真时**才走 `CrossSectionBuilder._compute_bar_metrics`（逐股逐日重算技术指标）。关键性质：
- **F01/MicroValue 只用 `market_cap`（基本面），`uses_bar_history=False`，不触发重算 → F01 可投性结论不受 B7 影响。**
- B7 只影响**用技术指标的策略**（return_20d/macd/rsi/…）。`_compute_bar_metrics` 的 `return_20d` 口径有 bug
  （`(closes[-1]-closes[0])/closes[0]`，`closes[0]` 是 ~119 根窗口最旧值 → 实为 ~119 日收益误标），feature_engine v1 已修。
  故 B7 是**行为变更的纠错**（回测数字会变，朝正确方向），非位等价重构。

**这比"换数据源"大**：`strategy_runner` 是回测与**实盘共用**的 runner。离线回测可读 `stock_features`；
但**实盘当日特征不在库**（离线算的），必须当场算。所以 B7 需一层**特征源抽象**（有库则读、无则用
`feature_engine`（向量化纠错版）当场算），并替掉 `_compute_bar_metrics`。涉及实盘链路，按 `docs/rules/live-trading.md`
须谨慎，宜独立专注一轮做。验证：离线读库值 == feature_engine 当场算值（feature_engine 已有等价测试）+ 回测跑通 + 抽查。
...
- **B7（回测重算）**：✅ **Phase 2 已关闭**（commit 44d4bed 等）。回测/截面 runner 不再用手写
  `_compute_bar_metrics`（return_20d 口径 bug），统一到 `feature_engine`（= stock_features 同引擎）：
  `SnapshotFeatureSource` 抽象 = `FeatureEngineFeatureSource`（当场算, 默认, 实盘/无库可用）+
  `StoredFeatureSource`（离线读已入库 stock_features, 复用别重算, 可注入）。`build_cross_section` 增
  `precomputed_features` 路径；runner 喂完整窗口(末根=T)→ feature_engine 末行 shift→as-of T-1, 无前视。
  定点差异验证（仅 return_20d 变, 余技术列对齐）+ 无前视测试 + 全套件绿。F01/MicroValue
  `uses_bar_history=False` 不触发 → 可投性结论不变。
