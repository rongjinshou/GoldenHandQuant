...
mvn -s maven-settings.xml -f code/pom.xml install -DskipTests   # 确认基线能构建
```
...
> 完整的逐模块发现母版见 `work/fixer/knowledge-base/findings.md`（本作品共 97 项：definite 80 / suspicious 17）。重跑本技能时以「重新审出的 findings」为准，不要照抄旧结论。

### 步骤 3 — 本地实修 + 验证（每条 finding 都要）

1. 在 `code/` 里实际改代码修复该 finding。
2. 跑 `mvn -s maven-settings.xml -f code/pom.xml test`（含针对**无黑盒覆盖**模块新增的单元测试，用于锁定修复），再 `install -DskipTests` 后跑 `mvn -s maven-settings.xml -f test-cases/pom.xml test`（24 个公开黑盒用例应逐步转绿）。
...
# (1) 改动(含新增)文件 → 复制完整最终内容到 KB，并记录其在基线的 SHA-256
git -C <repo> diff --name-only --diff-filter=ACMR "$BASELINE"..HEAD -- code/ | while read -r rel; do
  mkdir -p "$KB/$(dirname "$rel")"
  cp "<repo>/$rel" "$KB/$rel"
  base=$(git -C <repo> show "$BASELINE:$rel" 2>/dev/null | sha256sum | awk '{print $1}')
  [ -n "$base" ] && printf '%s  %s\n' "$base" "$rel" >> "$KB/baseline-hashes.txt"   # 新增文件在基线不存在则无 base 行，apply.sh 会走 [ADD]
done

# (2) 删除文件 → 记录其在基线的 SHA-256 到 deletions.txt
...
`baseline-hashes.txt` / `deletions.txt` 的行格式必须是 `<sha256>` + 两个空格 + `<相对路径>`（`apply.sh` 用 `awk '$2==p'` 精确匹配第二字段）。最后手写 `findings.md`：把每条 finding 整理成「症状 / 设计依据 / 修复 / 改动文件」表格，供评审抽查。
