...
4. **20 批（B01–B19 及 B05b）必须全部执行完——"当前用例全绿"不是完成条件，本任务也不存在"时间限制"。**
...
> 逐批跑一遍产物核验拿到机械事实：
> `for b in B01 B02 B03 B04 B05 B05b B06 B07 B08 B09 B10 B11 B12 B13 B14 B15 B16 B17 B18 B19; do bash work/harness/check-batch.sh $b | tail -1; done`
...
打开 **`work/bugs/README.md`**：它给出**批次执行顺序表**（B01→B19 及拆分批 B05b，共 20 批；公开用例受影响的模块在前、
...
> 小节即可，subagent 会自己打开。
>
> 这不是风格建议，是**整个运行能否跑完的前提**。卡片单个 25–120 KB；你每批读一个，全程就是
...
a. 从批次表查出该批对应的卡片文件名与小节（多数批次 = 一个文件；order/payment/S2-events
   ...
   **无论最终派的是哪种 agent 类型，派遣 prompt 第一行都必须是（把 <R> 替换为第⓪步锚定
   ...
   这是 20 批全部跑完的前提。**注意这份隔离只在"你自己不读卡片"时才成立**——你若先 read 一遍
...
```bash
bash work/harness/ratchet.sh verify    # 本步唯一一次终验：确保交付态 = 已验证良好态
bash work/harness/ratchet.sh status    # 打印最终 best，写进 result/output.md（total 见 verify 输出）
```

> **终验只跑这一次。**若它显示 best = total 且下方补救条件一条都未触发，**此后直到写完
> `STATUS: DONE` 都绝不再跑 verify**——status/check-batch/写记录都不改代码，本次结果一直有效；
> 重复终验只是白烧十几分钟构建（实测曾有收尾阶段空跑 3 次全绿 verify、浪费 18 分钟的先例）。
> 唯一需要再跑 verify 的情形：补救循环**真的重开了某批并改了代码**——那属于第③步 b 的动作，
> 跑完那一次后同样立即收笔，不再"确认性"重跑。
...
1. `ls code/pom.xml` 存在，且最近一次 verify（= 本步顶部那一次；若补救循环重开过批次则为
   重开后那一次）的 RATCHET_RESULT 是 ADVANCED / OK / OK_NO_CHANGES——**核对已有输出即可，
   绝不为"确认新鲜"重跑 verify**；
2. `result/output.md` 顶部『本次评测运行记录』节内批次表全部 20 批（B01–B19 及 B05b）每批一行（含未完成卡字段）；
...
1. 批次表全部 20 批（B01→B19 及 B05b）每批都已执行——合法的批次终态只有三种：已固化 / `ALREADY_APPLIED`
...
3. `result/output.md` 顶部『本次评测运行记录』节内含批次表全部 20 批（B01–B19 及 B05b）共 20 行批次记录（缺行 = 判定 1
