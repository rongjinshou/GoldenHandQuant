...
4. **19 批必须全部执行完——"公开 24/24" 不是完成条件，本任务也不存在"时间限制"。**
   公开 24 例只是**评分的一小部分**：真正的评分大头是数量更多的**非公开隐藏用例**（README §8
   明说公开用例只是子集），而 B07 之后的批次正是隐藏分的主要来源——**公开全绿后弃跑剩余批次
   = 主动放弃大部分分数**。绝不允许以"时间受限 / 剩余批次太多 / 上下文太长 / 已达 24/24 /
...
- 需要 `bash`、`JDK 17`、`Maven 3.6+`（用 `java -version` / `mvn -version` 确认；已具备则跳过
  下面的 JDK/Maven 安装块）。无需 Docker / 数据库 / 常驻服务（黑盒测试自带内存 H2；
  仅 Maven 下载依赖需要网络）。`work/` 本身纯文本 + bash，无三方依赖。
- 若缺 JDK/Maven，用下面的免 root 便携安装（不依赖包管理器、无需人工确认）：

```bash
mkdir -p "$HOME/tools" && cd "$HOME/tools"
# JDK 17 (Temurin 便携版)
curl -fsSL -o jdk.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.19%2B10/OpenJDK17U-jdk_x64_linux_hotspot_17.0.19_10.tar.gz
tar -xzf jdk.tar.gz
# Maven 3.9.9 (便携版)
curl -fsSL -o mvn.tar.gz https://archive.apache.org/dist/maven/maven-3/3.9.9/binaries/apache-maven-3.9.9-bin.tar.gz
tar -xzf mvn.tar.gz
# 环境变量落盘到 env.sh：export 只在当前 shell 有效，落盘后任何新 shell 都能 source 恢复
printf 'export JAVA_HOME="$HOME/tools/jdk-17.0.19+10"\nexport PATH="$JAVA_HOME/bin:$HOME/tools/apache-maven-3.9.9/bin:$PATH"\n' >"$HOME/tools/env.sh"
source "$HOME/tools/env.sh"
cd - && java -version && mvn -version    # 均能打印版本即就绪（注意回到原工作目录再继续）
```

  若你的每条命令在**新 shell** 里执行（多数 agent 的 bash 工具如此），后续手动 `mvn` 前先
  `source "$HOME/tools/env.sh"`；`work/harness/` 下的脚本会**自动探测** `$HOME/tools` 便携安装，
  不 source 直接跑也行。**注意：source env.sh 仅限主 agent（你）使用**——修复 subagent 的
  命令行**不得**出现 `$HOME` 等工程外路径（headless 下子会话的外部目录权限询问无人应答，
  会让整个运行永久挂死，见 SKILL「边界」首条），其 Maven 一律经
  `bash work/harness/mvnw.sh <参数>`（与 ratchet 同源自动探测，无需 source）。
...
```bash
bash work/harness/install-agent.sh
```

  注意：OpenCode 仅在启动时加载 agent 清单——本注册只对**之后新启动**的会话生效；你当前
  会话内直接按第 ③ 步 a 的递降链派遣即可，派不出 bug-fixer 不是安装失败。提交包内已预置
  `.opencode/` 注册目录——若你的运行时启动于本目录，bug-fixer 开箱可派；install-agent.sh
  仍需执行（幂等，兼顾从其他目录启动的运行时）。
...
它是只读的——**复制到当前工作目录后改副本，不要动原路径**。复制完成后当前目录下应出现
...
> 只要检测到下述状态就走续跑流程）：
> 若当前目录已存在 `.ratchet/golden-code`（`bash work/harness/ratchet.sh status` 能打印
> golden 与 best），说明这是续跑而非首跑——**跳过本步的源码复制**（重新复制会把工作树打回
> 未修复状态；棘轮虽能在下次 verify 自动回滚恢复 golden，但白白多付一轮构建与重试）。改为：
> ① 直接跑一次 `bash work/harness/ratchet.sh verify` 让工作树对齐 golden；② 打开
> `result/output.md` 顶部『本次评测运行记录』，从**最后一行已记录批次的下一批**继续第③步
> （已记录为固化/ALREADY_APPLIED 的批不再重做；最后一批若只 verify 未记录，先跑
> `check-batch.sh` 核验该批产物，OK 即补记录后继续）。
...
结构性高危改动殿后）与每张卡片的字段说明。对每一批，严格执行 a→b→c→d：

```text
a. 按批次表打开该批对应的卡片文件（多数批次 = 一个文件；order/payment/S2-events 一个文件
   含两个批次，各批只做批次表指到的那个 §A/§B 小节），逐卡修改 code/ 下的源文件
   —— 必须派 subagent 执行本批，派遣方式按下面的三级递降链逐级尝试（不允许跳级）。
   **无论最终派的是哪种 agent 类型，派遣 prompt 第一行都必须一字不差地是：
   「先完整阅读 work/skills/bug-fixer/SKILL.md 并遵守其全部规范，然后执行：修复
   work/bugs/<文件> 的 <小节>，工程在当前工作目录 code/ 下」**（bug-fixer 已内置该规范，
   重复要求无害；general 则完全依赖这一行——漏写它的后果是 subagent 不知道 mvnw.sh /
   test-compile / 外部路径禁令等全部纪律）。每次派遣前自查 prompt 是否以该行开头。
   ...
   这是 19 批全部跑完的前提。**一次只允许存在一个在途 subagent。**绝不并行派遣多个批次、
...
是只读诊断（编译 + 24 例，不动工作树），排查问题时可用。

### 第 ④ 步 — 收尾（必须做）：终验 + 补救循环（best<24 时同样必须）

```bash
bash work/harness/ratchet.sh verify    # 最后一次护栏：确保交付态 = 已验证良好态
bash work/harness/ratchet.sh status    # 打印最终 best 通过数，写进 result/output.md
```

**补救循环（best < 24 时是必做项，不是可选项；所有动作都受棘轮保护、只可能加分不可能倒退。
补救循环的重开是新的一轮：重开的批重新获得一次重试额度，仍受"重试一次、再败跳过"约束）：**

- 若 `status` 显示 best **< 24**：**必须**执行补救循环——从 `.ratchet/test.log` / `check-all.sh`
  找出失败的 `pubNNN_*` 用例，映射到模块，对相关批次**再走一轮**第③步 a→b→c→d。**绝不允许**
  把失败用例解释成"弃项 / 修复范围外 / 特殊情况"而放弃补救——24 个公开用例全部在卡片覆盖
  范围内，公开用例失败只有一个含义：对应批次的某张卡没修对或没修全（重开该批时逐卡核对
  「验收」条目找出漏改的那张）。
...
- 若 best = 24：对照 `work/checklist/<module>.md` 逐模块扫读一遍是可选加分动作——它是本任务
  中**唯一**的可选动作，其余一切步骤均为必做。发现某条强规则仍不满足（说明有卡漏改/改偏）→
  回到对应批次按 a→b→c 补修一轮。隐藏用例只考设计契约，这一步是公开用例覆盖不到的提分空间。

最后补全 `result/output.md` 顶部『本次评测运行记录』节：基线通过数、最终通过数、跳过批次汇总
及原因（每批的 RATCHET_RESULT 行应已在第 ③ 步 d 逐批追加过），末尾一行 `STATUS: DONE`。
...
1. 批次表 B01→B19 每批都已执行——合法的批次终态只有三种：已固化 / `ALREADY_APPLIED`
   ...
   见心法第 4 条）。**"已达 24/24"不构成跳过理由**，公开全绿后剩余批次仍必须逐批执行；
...
- **修复后的工程**：`<当前工作目录>/code/`（golden 已同步，`.ratchet/` 为护栏状态目录，可忽略）。
