...
- **一批 BUG 卡片**（来自 `work/bugs/<batch>.md`），每张卡含：
  ...
  **勿犯**（高危卡才有：此卡最容易引入的事故）。
- 待修复工程在**工程根目录 R** 下的 `code/`。**R 的绝对路径由派遣指令第一行给出**——你的
  会话初始目录很可能是与工程无关的全局根目录，**开始任何操作前先 `cd <R>`**；本文一切
  相对路径（`code/`、`work/…`、`maven-repo`）都以 R 为基准，每条命令都在 R 下执行（新开
  命令时如不确定就再 `cd <R>` 一次）。若派遣指令没给 R，向上追溯你被要求阅读的本文件路径，
  其上上级的上级目录即 R（本文件位于 `<R>/work/skills/bug-fixer/SKILL.md`）。
...
1. **通读本批全部卡片**再动手——同批卡片可能触及同一文件，先建立全貌避免相互覆盖。
   **一次读完、只读一遍**：卡片文件普遍 1000~2100 行，而 read 工具默认只给 500 行；用
   `limit`（给 3000）+ 必要时 `offset` 顺序读完，读过的段落**不要再读第二遍**（实测有过
   同一卡片文件被整体读两遍、白烧掉一倍上下文的情况）。你的上下文也有上限，读重了会在
   整批结束前耗尽，导致本批半途而废、被棘轮判回滚。
