...
第三轮深审·跨领域 #1（`markUsed` 缺归属校验）。本批（B05）执行 **17 张卡**：PROMO-1…13、17…20。「订单取消释放」族 PROMO-14/15/16（含内嵌的
order 侧接线卡 ORD-A17/ORD-A21）**已拆至 `promotion-release.md`（批次 B05b，紧随本批）**——
本批跳过这三个编号的墓碑条目即可，勿打开那个文件。PROMO-17/18/19/20 为契约复核后补卡——满减
创建双形态兼容、`applicableCoupons` 组装、创建入口防护、calculate 响应别名——每张均已在参考
实现上通过全部 24 例公开黑盒门禁。
...
4. **本批 17 张卡必须一次性全部改完后才 verify——严禁批中 verify**：PROMO-8 落地而 PROMO-11 未
...
- **本卡不在 B05 执行**：已拆至 `promotion-release.md`（批次 **B05b**，紧随本批之后执行）。
  B05 的 subagent 跳过本条目即可，勿去打开那个文件。

---

### PROMO-15 | 订单取消后秒杀名额从不归还（购买记录缺 orderId，无从反查）

- **本卡不在 B05 执行**：已拆至 `promotion-release.md`（批次 **B05b**）。跳过本条目，勿打开该文件。

---

### PROMO-16 | 券/秒杀释放的 order 侧接线（内嵌 ORD-A17 与 ORD-A21 完整卡）

- **本卡不在 B05 执行**：已拆至 `promotion-release.md`（批次 **B05b**），内嵌的 ORD-A17/ORD-A21
  随之同移。跳过本条目，勿打开该文件。
