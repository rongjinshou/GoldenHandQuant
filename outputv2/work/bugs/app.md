...
本文件覆盖 `findings.md`「app 模块（§6.12）」全部 4 项 + 第二轮深审（§7）#2 + 第四轮契约审核追加项，共
**4 张卡**（APP-1..4）。APP-4 是支撑端点错误体的 app 侧收口，与 B11/COMMON-4 配套（common 侧 handler 先行，
app 侧控制器与过滤链级单测在本批完成）；APP-1 的整文件替换目标已同步包含 COMMON-4 对
`SystemAdminController` 错误路径的修订，两批任意先后均收敛。
...
- **改法**:
  ...
  2. 同步删除因此变成完全未使用的字段/构造参数/常量/import：`applicationContext`（`ApplicationContext`）、
     ...
     import 与字段——`configs`/`clock` 四个端点的成功路径原样不动。删完后类里不再需要任何依赖注入，构造函数直接消失
     （使用默认无参构造）。
     **注意**：下方目标文件已同步包含 B11/COMMON-4 对本文件错误路径的修订（标准契约错误体 + `offsetMinutes`
     防御解析，import 相应增加 `ValidationException`/`ResourceNotFoundException`）——B11 与 B12 无论先后
     执行，本文件最终都收敛到同一文本；若替换前发现文件已含 COMMON-4 的部分内容，照常整文件替换即可，结果
     幂等。改完后的目标文件内容（可逐行核对）：
     ```java
...
- **验收**: `POST /api/v1/admin/system/reset-sandbox` 与 `POST /api/v1/admin/system/bootstrap-admin` 均返回
  **404**（路径不存在，不是 401/403——因为 handler 已被整个删除，不是"改成需要鉴权"）；
  ...
  `GET/PUT /api/v1/admin/system/configs/{key}`、`PUT/DELETE /api/v1/admin/system/clock` 四个端点成功路径行为
  完全不变（仍需 ADMIN 角色；错误路径按 B11/COMMON-4 统一为标准契约错误体，已体现在上方目标文件中）；`mvn -f code/pom.xml compile` 无未使用 import/参数相关的编译错误。
...
### APP-4 | 支撑端点错误路径不走契约错误体——app 侧收口（与 B11/COMMON-4 配套）

- 风险: low · 置信度: definite
- **文件**:
  1. `code/ecommerce-app/src/main/java/com/ecommerce/app/controller/FaultInjectionAdminController.java`
  2. （同步单测）`code/ecommerce-app/src/test/java/com/ecommerce/app/config/SecurityConfigTest.java`
- **现状**: `FaultInjectionAdminController.injectFault` 缺 `fault` 名时手搓非契约响应
  `ResponseEntity.badRequest().body(Map.of("error", "fault name is required"))`——错误响应必须是
  `{code, message, traceId, details}`（design-docs/03；design-docs/01 明确黑盒支撑接口纳入 API 契约）。
  `SystemAdminController` 的同类问题（含 `offsetMinutes` 的 `(Number)` 盲强转会把字符串载荷变 500）**已并入
  APP-1 的整文件替换目标**，不在本卡重复施工。`SecurityConfigTest`（基线既有）尚无"认证通过后访问未知路径
  → 404 契约体"的过滤链级用例——对应 handler 由 B11/COMMON-4 在 `GlobalExceptionHandler` 新增
  （`NoResourceFoundException` → 404 `RESOURCE_NOT_FOUND`）。
- **期望**: 支撑端点错误路径全部走标准异常：缺 `fault` 名 → 400 `{code:"VALIDATION_FAILED",...}`。依据:
  README §7.1、design-docs/03（错误响应格式）、design-docs/01。**成功路径逐字节不变。**
- **改法**:
  1. `FaultInjectionAdminController`：import `com.ecommerce.common.exception.ValidationException`；
     `injectFault` 里 `faultName == null || faultName.isBlank()` 分支改为
     `throw new ValidationException("fault name is required");`（替代手搓 badRequest）。成功分支
     （POST 200 `{activeFaults}`、DELETE 204 空体）一行不动。
  2. `SecurityConfigTest`：内部 `TestConfig` 的 `@Import` 列表追加 `GlobalExceptionHandler.class`（+对应
     import 与 `jsonPath` 静态 import），新增用例：带合法 USER token
     `GET /api/v1/definitely-not-a-real-endpoint` → 404 + `$.code=RESOURCE_NOT_FOUND` +
     `$.message=Resource not found` + `$.traceId` 非空（穿过真实安全过滤链 + DispatcherServlet 的行为级验证）。
- **验收**:
  1. 带 ADMIN token `POST /api/v1/admin/ops/fault-injections` body `{}` → 400 `{code:"VALIDATION_FAILED",...}`
     （改前是 `{"error":...}`）；`DELETE /api/v1/admin/ops/fault-injections` → 204 不变。
  2. `grep -qF "ValidationException" code/ecommerce-app/src/main/java/com/ecommerce/app/controller/FaultInjectionAdminController.java`
     命中（artifacts.tsv B12 断言）；`grep -qF "parseOffsetMinutes" .../SystemAdminController.java` 命中
     （验证 APP-1 的新目标文本落地，artifacts.tsv B12 断言）。
  3. `mvn -f code/pom.xml test -pl ecommerce-app -Dtest=SecurityConfigTest` 全绿。
  4. 黑盒回归 24/24（PUB-108 打 fault-injections 成功路径）。
- **勿犯**:
  1. **执行顺序**：本卡排在 APP-1/APP-3 之后（同批内按卡片顺序执行即天然满足）——`SecurityConfigTest` 的
     `@Import` 列表由 APP-3 先行加入两个新类，本卡在其基础上追加；新增用例依赖 B11/COMMON-4 的
     `NoResourceFoundException` handler（B11 在 B12 之前，正常已就位）。若发现 `GlobalExceptionHandler`
     尚无该 handler（B11 异常跳过的罕见情形），第 2 步暂缓并在批报告注明，第 1 步照做。
  2. 不要动 `SystemAdminController`——它的错误路径修订已包含在 APP-1 的整文件目标文本里，本卡无需重复。
  3. PUB-108 直接依赖 fault-injections 成功路径——POST 200 `{activeFaults}` 与 DELETE 204 的响应形状不得
     有任何变化。
