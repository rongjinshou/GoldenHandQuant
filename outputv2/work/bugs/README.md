...
## 批次执行顺序（严格按下表行序执行，B01→B19 中含拆分批 B05b、共 20 批；每批修完必跑 `bash work/harness/ratchet.sh verify`）
...
| B05 | `promotion.md` | 优惠计算 / 秒杀（含事务毒化修复）/ 券核销归属 / 满减与响应契约（公开 pub101 在此） | mid |
| B05b | `promotion-release.md` | 券/秒杀取消释放（PROMO-14/15）+ order 侧接线（PROMO-16 内嵌 ORD-A17/A21） | low |
...
依赖关系：**B05b（promotion-release）依赖 B05**（释放建立在核销 orderId 与秒杀购买留痕之上，
B05 被跳过时 B05b 顺延到补救循环一起重开）。**B14（logistics）、B15（loyalty）、B16（S2 §B）都依赖 B13（S2 §A）建好的
