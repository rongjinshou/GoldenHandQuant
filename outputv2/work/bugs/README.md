...
失败代价大的放在公开分锁定之后做。**注意：公开 24 例只是评分的一小部分，数量更多的非公开
隐藏用例才是评分大头，而 B07 及之后的批次正是隐藏分的主要来源——公开全绿（24/24）之后，
...
| B05 | `promotion.md` | 优惠计算 / 秒杀（含事务毒化修复）/ 券核销归属（公开 pub101 在此） | mid |
...
| B14 | `logistics.md` | 拣货-面单-出库链 / 回调幂等 / 运费模板 | mid |
...
1. **不给 `OrderLogisticsStatusUpdater` 写生产实现。** 冻结的 `test-cases/BlackboxHarnessConfig`
   已注册一个无限定符的该接口 no-op bean；在 `ecommerce-order` 生产代码里新增实现必然
   `NoUniqueBeanDefinitionException`，Spring 上下文启动失败，**24 例全灭**。
...
3. **不新建 Spring `CacheManager` bean。** 全系统唯一真正实现 `org.springframework.cache.CacheManager`
   ...
   上下文启动失败，**24 例全灭**——这正是上一轮评测实际发生过的事故。
...
6. **不实施卡片之外的任何"发现"。** 你若觉得"还有个 bug 卡片没写"，把它记进 `result/output.md`
   ...
   明确弃项（如运费模板接线、优惠券取消释放、DELIVERED→COMPLETED 触发等）。
