...
```bash
# 一键验收链（推荐: ruff全仓 + pytest全量 + 前端产物哈希 + 数据质量门禁, ~30s;
# --frontend 附加 vitest/typecheck; 全绿退出 0）
$WIN_PYTHON scripts/verify_all.py

# 运行全部测试（gateway 测试已回归标准命令: Windows 下全跑,
# WSL 无 xtquant 时由 tests/infrastructure/gateway/conftest.py 优雅跳过）
...
# 运行 ruff lint（src/tests/scripts 全部纳入门禁, 全仓 0 违规基线 2026-07-10）
...
# 数据质量门禁（NULL固化哨兵/新鲜度/对齐; 有 FAIL 退出码 1, 进验收链）
$WIN_PYTHON -m src.interfaces.cli.quant data status --check

# 交易所日历同步（trade_calendar 表, 含未来节假日→影子盘/时段闸前瞻; 年度维护:
# 每年 12 月重跑; 需 TUSHARE_TOKEN 环境变量; WSL 可跑; 设计 docs/feat/0711-tushare-asset/）
python scripts/sync_trade_calendar.py
# tushare 数据收割器（落独立 data/tushare.duckdb, 断点续传; 运行时系统零依赖 tushare）
python scripts/tushare_harvest.py --interface daily_basic --mode date --start 2020-01-01

# 市值口径（MC-1, 2026-07-12 起 market_cap=时点总市值, 原 QMT 口径备份于 market_cap_qmt）
python scripts/migrate_market_cap.py        # 历史迁移(幂等, 一次性已跑)
python scripts/sync_market_cap.py           # 日增量: 最新交易日覆写(tushare→akshare 兜底,
                                            # 双败退出 1; 影子盘编排上午段自动跑)
python scripts/refresh_instrument_names.py  # instruments 名称对齐交易所最新名(C9 观察项配套)
...
# 影子盘每日编排（QMT 看护提醒→refresh→采样→收盘比对+净值; 2026-07-14 起日频半个月,
# 任务计划 GHQ-Shadow-Morning/PostClose 工作日 09:20/15:10 经 scripts/windows/shadow_*.bat 自动跑;
# 调仓语义仍按周二, 其余日主要看防御信号; 设计 docs/feat/0711-shadow-control/）
$WIN_PYTHON scripts/shadow_tuesday.py --force                 # 上午段(--force=非周二也采)
$WIN_PYTHON scripts/shadow_tuesday.py --force --post-close    # 收盘段
# 影子盘过程仪表（采样台账/G1 有效样本≥10 交易日含≥2 调仓周二/判据 G1-G7; 异常退出码 1; WSL 可跑）
...
$WIN_PYTHON scripts/seed_paper_trading.py --yes   # 清空生产 data/trading.db 须显式 --yes; 演示请 --db 指向别的库
